...
def ObjectiveRsquared(trial:Trial, xtrain=None, xval=None, ytrain=None, yval=None):
    rf_n_estimators = trial.suggest_int('n_estimators', 10, 20, step=5)
    rf_max_depth = trial.suggest_int('max_depth', 4, 10)
    rf_max_features = trial.suggest_categorical('max_features', ['sqrt']) #['sqrt', 'log2']
...
def OptimalHyperParameters(objective, xtrain, xval, ytrain, yval):
    ...
    study.optimize(partial(objective, xtrain=xtrain, xval=xval, \
                       ytrain=ytrain.values, yval=yval.values), n_trials = 100, \
               n_jobs = -1)
